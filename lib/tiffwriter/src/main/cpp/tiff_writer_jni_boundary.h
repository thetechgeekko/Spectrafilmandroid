#ifndef SPECTRAFILM_TIFF_WRITER_JNI_BOUNDARY_H
#define SPECTRAFILM_TIFF_WRITER_JNI_BOUNDARY_H

#include <cstdint>
#include <exception>
#include <new>

namespace spectrafilm::tiffjni {
...
inline const char* stableMessage(NativeExceptionKind kind) noexcept {
    switch (kind) {
        case NativeExceptionKind::OutOfMemory:
            return "TIFF write failed: out of memory";
        case NativeExceptionKind::Standard:
            return "TIFF write failed: native exception";
        case NativeExceptionKind::Unknown:
            return "TIFF write failed: unknown native exception";
...
#endif  // SPECTRAFILM_TIFF_WRITER_JNI_BOUNDARY_H
