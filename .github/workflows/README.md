...
| **native-safety-writers** | One committed runner performs the exact ten-suite host inventory: six ASan+UBSan rows for JNI safety helpers, bounded JSON/NPY parsing, an asset-backed engine C render/cancel race, and PNG/TIFF hostile-input/JNI-helper tests; four TSan rows repeat the synchronized engine race and cover the allocation registry and writer cancellation races. This does **not** claim sanitizer runtime coverage of the actual engine JNI bridge. |
...
`bash tools/release/run_native_safety.sh --list` prints the locked six
...
qualification calls CI's same committed ten-suite ASan+UBSan/TSan runner for
JNI safety helpers, bounded JSON/NPY parsing, the synchronized asset-backed engine C cancellation
race, and PNG/TIFF writer gates on
