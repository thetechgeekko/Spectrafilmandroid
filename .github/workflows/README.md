...
| **native-safety-writers** | One committed runner performs the exact eight-suite host inventory: ASan+UBSan for JNI safety helpers, an asset-backed engine C render/cancel race, and PNG/TIFF hostile-input/JNI-helper tests; TSan repeats that synchronized engine race and covers the allocation registry and writer cancellation races. This does **not** claim sanitizer runtime coverage of the actual engine JNI bridge. |
...
`bash tools/release/run_native_safety.sh --list` prints the locked four
ASan+UBSan plus four TSan inventory. Running it without arguments compiles and
...
qualification calls CI's same committed eight-suite ASan+UBSan/TSan runner for
JNI safety helpers, the synchronized asset-backed engine C cancellation race,
and PNG/TIFF writer gates on
